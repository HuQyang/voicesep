...
def run_vad_firered(wav_path: str, model_dir: str = "pretrained/FireRedVAD/VAD"):
    ...
    # 兼容 FireRedVAD 的调用接口
    try:
        # 大多数情况下 API 是 __call__ 或者 predict/infer
        if hasattr(_firered_vad, "predict"):
            raw_results = _firered_vad.predict(wav_path)
        elif hasattr(_firered_vad, "infer"):
            raw_results = _firered_vad.infer(wav_path)
        else:
            raw_results = _firered_vad(wav_path)
    except Exception as e:
        print(f"  [vad-fail] FireRedVAD 推理失败: {e}")
        return []

    segments_ms = []
    # 统一转换输出格式为 [(start_ms, end_ms), ...] 喂给后续的声纹和 BSS 模块
    for res in raw_results:
        # 根据官方 API 输出可能返回字典或元组列表，这里做鲁棒兼容
...
def main():
    ...
    ap.add_argument("--vad", choices=["fsmn", "silero", "firered"], default="fsmn",
